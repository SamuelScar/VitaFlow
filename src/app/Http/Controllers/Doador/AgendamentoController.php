...
class AgendamentoController extends Controller
{
    ...
    private function validarIntervaloMinimoDoacao(User $user): Closure
    {
        return function (string $attribute, mixed $value, Closure $fail) use ($user): void {
            ...
            $dataReferencia = $this->conflitoIntervaloDoacao(
                $this->datasReferenciaIntervaloDoacao($user),
                $intervaloDias,
                $dataHora,
            );
     ...
     */
    private function horariosAgendamento(Campanha $campanha, CarbonImmutable $inicio, CarbonImmutable $fim, User $user): array
    {
        $ocupacao = $campanha->agendamentos()
            ->where('status', 'agendado')
        ...
        $datasReferencia = $this->datasReferenciaIntervaloDoacao($user);
        $intervaloDias = $user->intervaloMinimoDoacaoDias();
        $horarios = [];
     ...
     */
    private function datasReferenciaIntervaloDoacao(User $user): array
    {
        ...
        $agendamentosAtivos = DB::table('agendamentos')
